...
## Length & components
- **1,200–2,500 words.**
- Post opens with: `<Blockquote>` (2–3 sentence summary) → MAIN `<Image>` → **first `## <Highlighter>` heading** → intro paragraphs → rest of the sections.
- **NEVER put prose between the main `<Image>` and the first heading.** The first thing after the image is always a heading; the intro/hook paragraphs belong *under* that first heading, above its other content. A post that starts with a bare paragraph is wrong.
- `## <Highlighter>Section Title</Highlighter>` for every main heading (always with the `## ` prefix).
- **Sub-group labels: use `##### ` (h5).** When a section groups bullet points under short standalone labels (e.g. `##### Safety`, `##### When to seek urgent medical care`), format each label as an `##### ` heading, not plain text or a bold line - it keeps grouped lists organized. Leave a blank line after the heading before the bullets. (This is for standalone group labels; a normal sentence that ends in a colon and leads directly into a list stays as prose.)
...
## Writing standards
- Open the first section (under its heading, never before it) with 2–3 short paragraphs (no bullet list): a relatable scenario or question hook, acknowledge the reader's situation, state what they'll learn. Include one natural link.
